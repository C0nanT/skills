...
Every ticket is **HITL** (human in the loop — grilling, prototyping) or **AFK** (agent alone — researching); a HITL ticket only resolves through a live exchange, so the agent never answers its own questions.
