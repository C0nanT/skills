...
It also needs [grilling](https://aihero.dev/skills-grilling) present, because its own `SKILL.md` is one line that delegates the interview to it; the doc-writing itself is `grill-with-docs`'s own instruction, not a separate skill. Installing `grill-with-docs` without `grilling` gets you a skill that does not work.
...
That is the skill failing to load its dependency. Because `SKILL.md` is a one-line delegation, an agent that does not pick up [grilling](https://aihero.dev/skills-grilling) guesses at what grilling means, and you get an undifferentiated question dump with no paper trail. It correlates with model and [effort](https://www.aihero.dev/ai-coding-dictionary/effort) level, and it is the most reported problem with this skill. If you suspect it, ask the agent directly which skills it loaded.
