...
- **[setup-devcontainer](./setup-devcontainer/SKILL.md)**: Set up a Compose-based devcontainer that inherits the host's git, CLI and agent identity: the toolchain lives in the container, the host stays clean, and nothing needs re-authenticating. User-invoked.
- **[setup-pre-commit](./setup-pre-commit/SKILL.md)**: Set up Husky pre-commit hooks with lint-staged, Prettier, type checking, and tests. User-invoked.
