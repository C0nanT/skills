...
- **`/sync-upstream`**: this repo is a fork of [mattpocock/skills](https://github.com/mattpocock/skills), and pulling upstream in is a recurring job with a recurring hazard: the conflicts git marks are the easy half, while a file that merges *cleanly* can quietly restore a name you renamed or a behaviour you rewrote. It resolves the marked conflicts against `.agents/fork-divergences.md`, the written ledger of how this fork differs, then counts every upstream-side name across the tree to catch the silent ones, checks the repo's own invariants, and leaves the merge open for you. Reach for `/resolving-merge-conflicts` instead when there's no fork relationship in play.
...
- **`/review-mr`**: review someone else's merge request (GitLab, GitHub, or two local branches) for bugs, security, performance and design, and write the findings to a local markdown file. Where `/review-axes` judges your own diff against the repo's standards and spec, this judges someone else's.
