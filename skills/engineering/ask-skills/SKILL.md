...
3. **Branch — is this a multi-session build?**
   - **Yes** → **`/to-spec`** (turn the thread into a spec), then **`/to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges**. On a local tracker that's one file per ticket under `.scratch/<feature>/tickets/`, worked blockers-first by hand; on a real tracker the edges become native blocking links, so any ticket whose blockers are done can be grabbed — kick off **`/implement`** per ticket, **clearing context between each one**. To automate that clearing — one ordered ticket sequence, a fresh subagent per ticket, stopping the moment one fails — use **`/delegate-tickets`** instead of driving it by hand.
...
- **`/review-mr`** — you're the reviewer on **someone else's** merge request. Takes a GitLab MR, a GitHub PR, or two local branches, and reviews along four axes — **Correctness, Security, Performance, Design** — reading whole files and callers, not just the diff. Every finding must carry a `file:line` and the concrete execution path that produces it, or it's cut; an empty report is a valid result. Output is a markdown file under `.scratch/reviews/`, never a dump in the chat, and never a comment posted to the MR unless you ask for it. Where `/review-axes` asks "does my diff match the standards and the spec?", this asks "is this code, which I didn't write and have no spec for, going to break or leak something?"
