...
- **`setup-pre-commit` lives in `skills/in-progress/`, not upstream's `skills/misc/`.** This fork un-promoted it (no plugin.json entry, no top-level README line) to sit with the other beta skills. `EXCLUDED_PATHS` strips `skills/misc/setup-pre-commit` after a merge so upstream's copy does not reappear beside the in-progress one.
