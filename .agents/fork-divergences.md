...
| `review-axes` | **Performance baseline** on the Standards axis: six repeated-work patterns (N+1 queries, N+1 network calls, missing index, unbounded result set, repeated work in a loop, quadratic lookup), each finding naming the collection that drives the multiplier | a step 3 whose only baseline is the twelve Fowler smells |
| `review-axes` | **Duplicated Code covers three shapes**, including a hunk that re-implements something the repo already has, and the Standards brief tells the sub-agent to grep and cite the existing path | `the same logic shape appears in more than one hunk or file in the change. → extract` with nothing after it |
...
- **No automatic hand-off out of `diagnosing-bugs`.** Upstream deleted Phase 6's post-mortem (it called user-invoked `/improve-codebase-architecture`, which no skill can reach) and this fork follows it: Phase 6 is Cleanup only, byte-identical to upstream. What is fork-specific is the **prose around it**: `ask-skills` used to advertise the hand-off as automatic and was reworded to say the human runs that skill themselves once the fix is in. A merge that restores "this skill hands off there itself" is restoring a claim the skill no longer backs.
...
- **Dropped upstream paths** live in `scripts/sync-upstream.sh` (`EXCLUDED_PATHS`): that list is the ledger for deletions; add to it there. This fork also drops `setup-ts-deep-modules` (`skills/in-progress/setup-ts-deep-modules/`): it exists to wire dependency-cruiser around `codebase-design`'s deep-module vocabulary, and `codebase-design` is already excluded. A merge that recreates the directory is undoing this. The same list strips `skills/engineering/diagnosing-bugs` and `docs/engineering/diagnosing-bugs.md`: this fork still carries the skill, but in `skills/misc/` and unpromoted (see below). A merge that restores either engineering path is undoing the relocate.
- **These skills stay on disk but are not promoted** (no `.claude-plugin/plugin.json` entry and no top-level README line). `scripts/validate.sh` lists them in `UNPROMOTED_SKILLS` so the public-bucket checks do not demand them back into the plugin. Install with `--skill=<name>`. A change that re-lists one in `plugin.json` or the root README is undoing this.
  - `setup-pre-commit`: stays in `skills/misc/` (upstream's location).
  - `reset-agent-env`: fork-only, stays in `skills/misc/`. Used to be in the plugin and the root README Misc section; it is not, any more. A merge that puts `./skills/misc/reset-agent-env` back into `plugin.json` is undoing the unpromote.
  - `diagnosing-bugs`: lives in `skills/misc/`, not upstream's `skills/engineering/`. Unpromoted: absent from `plugin.json`, the root README, and `skills/engineering/README.md`. `EXCLUDED_PATHS` strips `skills/engineering/diagnosing-bugs` and `docs/engineering/diagnosing-bugs.md` after a merge so upstream's copies do not reappear beside the misc one. Sweep: `grep -n "skills/engineering/diagnosing-bugs" README.md skills/engineering/README.md .claude-plugin/plugin.json`.
...
- **`skills/misc/` is partially promoted, unlike upstream.** `setup-statusline` still appears in `.claude-plugin/plugin.json`'s `skills` array and in the top-level `README.md`'s Misc section, even though `CLAUDE.md`'s Invariants section currently only names `engineering/` and `productivity/` as promoted buckets. This predates this sync (not introduced by a merge): `CLAUDE.md`'s wording just hasn't caught up. Flagged here rather than silently "fixed" either direction; a human should decide whether to update the invariant text or un-promote that one too. `setup-pre-commit` and `reset-agent-env` used to sit in that promoted-misc set; `diagnosing-bugs` used to be a promoted engineering skill. All three stay in `misc/` but are unpromoted (see above).
