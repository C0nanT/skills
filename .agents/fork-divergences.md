...
- **`skills/misc/` is partially promoted, unlike upstream.** `setup-statusline` and `reset-agent-env` still appear in `.claude-plugin/plugin.json`'s `skills` array and in the top-level `README.md`'s Misc section, even though `CLAUDE.md`'s Invariants section currently only names `engineering/` and `productivity/` as promoted buckets. This predates this sync (not introduced by a merge): `CLAUDE.md`'s wording just hasn't caught up. Flagged here rather than silently "fixed" either direction; a human should decide whether to update the invariant text or un-promote those two. `setup-pre-commit` used to be in that set; it stays in `misc/` but is unpromoted (see above).
