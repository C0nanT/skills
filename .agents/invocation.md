...
The Skill tool takes one skill per call. A step that needs two skills is two calls, not one call with two names: say so (`Call the Skill tool twice, for "grilling" and "prototype"`), not "call it with X and Y," which reads as a single call taking both.
